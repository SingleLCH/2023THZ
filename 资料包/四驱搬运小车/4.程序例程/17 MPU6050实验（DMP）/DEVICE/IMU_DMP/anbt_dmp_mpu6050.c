...
{
	MPU_I2C_Init_IO();						//I2C³õÊ¼»¯
	
	RCC->APB2ENR|=1<<4;     //Ê¹ÄÜPORTCÊ±ÖÓ
	RCC->APB2ENR|=1<<0;     //¿ªÆô¸¨ÖúÊ±ÖÓ		  
	GPIOC->CRH&=0XF0FFFFFF; //PC14ÉèÖÃ³ÉÊäÈë	  
	GPIOC->CRH|=0X08000000;
	Ex_NVIC_Config(GPIO_C,14,FTIR); 			//ÏÂ½µÑØ´¥·¢
	MY_NVIC_Init(0,0,EXTI15_10_IRQn,2);    //ÇÀÕ¼2£¬×ÓÓÅÏÈ¼¶2£¬×é2
...
void run_self_test(char cal)
{
  ...
  long gyro_test[3]={0,0,0}, accel_test[3]={0,0,0};
	...
	{	
		...
		if( result == 0x3 )
		{
// 			Flash_Save_GyroOffset(gyro_test);//±£´æÐ£×¼ÊýÖµ
// 			Flash_Save_AccOffset(accel_test); //±£´æÐ£×¼ÊýÖµ
		}
	}
// 	Flash_Read_GyroOffset(gyro_test);//´ÓFlash¶Á³öÐ£×¼ÊýÖµ
// 	Flash_Read_AccOffset(accel_test); //´ÓFlash¶Á³öÐ£×¼ÊýÖµ
...
//			AnBT_Uart1_Send_Num(anbt_dmp_data[0]);
		...
		//
    anbt_dmp_data[0] = 0x80;	//etootle: BIT_RESET, Reset device			ÖØÉèÆ÷¼þ
    if (AnBT_DMP_I2C_Write(st.hw->addr, st.reg->pwr_mgmt_1, 1, &(anbt_dmp_data[0])))
			return 3;   
    AnBT_DMP_Delay_ms(100);
		//
    anbt_dmp_data[0] = 0x00; //etootle: Wake up chip			»½ÐÑ
    if (AnBT_DMP_I2C_Write(st.hw->addr, st.reg->pwr_mgmt_1, 1, &(anbt_dmp_data[0])))
			return 4;
    //
    if (AnBT_DMP_I2C_Read(st.hw->addr, st.reg->accel_offs, 6, anbt_dmp_data))
			return 5;	//etootle:Check product revision  		¼ì²é²úÆ·ÐÞÕý
...
//				AnBT_Uart1_Send_String("Unsupported version=1.0",23);  
      ...
      if (!anbt_dmp_rev) 
			{
//				AnBT_Uart1_Send_String("Product ID error",16); 		//²úÆ·±àºÅ´íÎó
				return 5;    
      } 
...
//				AnBT_Uart1_Send_String("Half Sensitivity Part",21); 
    ...
    if (mpu_set_bypass(0)) return 0x15;					//²»Í¬µã
