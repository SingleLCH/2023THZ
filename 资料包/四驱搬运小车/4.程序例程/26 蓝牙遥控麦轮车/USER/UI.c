...
"[ 2.4G遥控 ]\0",	"[ 2.4G遙控 ]\0",	"[2.4G CTRL ]\0",	//13
};

unsigned char StandbyUI_String[3][8][7]={
"前左：\0","前右：\0","后左：\0","后右：\0","电压：\0","蓝牙：\0","断开  \0","连接  \0",
"前左：\0","前右：\0","後左：\0","後右：\0","電壓：\0","藍牙：\0","斷開  \0","連接  \0",
"F_L： \0","F_R： \0","B_L： \0","B_R： \0","Volt：\0","BLUE：\0","broke \0","link  \0",
};
...
void Standby_Mode(void)
{
	...
	{
		OLED_ClearBuffer(0);
		OLED_Put12x12CNstr(24,0,Title_Strings[WorkMode][Lang_x],LIGHT);
		OLED_Put12x12CNstr(0,13,StandbyUI_String[Lang_x][0],LIGHT);			OLED_Put12x12CNstr(66,13,StandbyUI_String[Lang_x][1],LIGHT);
		OLED_Put12x12CNstr(0,26,StandbyUI_String[Lang_x][2],LIGHT);			OLED_Put12x12CNstr(66,26,StandbyUI_String[Lang_x][3],LIGHT);
		OLED_Put12x12CNstr(0,39,StandbyUI_String[Lang_x][4],LIGHT);			OLED_Put12x12CNstr(66,39,StandbyUI_String[Lang_x][5],LIGHT);
		OLED_Put12x12CNstr(0,52,StandbyUI_String2[WorkMode][Lang_x],LIGHT);			//显示提示语
	...
	if(Kaysta==2)	//按键长按
	{
 	...
 	OLED_Put6x12Num(32,13,Encoder[3],5,LIGHT);	OLED_Put6x12Num(98,13,Encoder[2],5,LIGHT);		//显示角度MPU6050_accel[2]
	OLED_Put6x12Num(32,26,Encoder[0],5,LIGHT);	OLED_Put6x12Num(98,26,Encoder[1],5,LIGHT);	//显示转速
	OLED_Put5x7Num1dot(30,42,Volt);																								//显示电池电压
	if(BlueCK==0)	OLED_Put12x12CNstr(98,39,StandbyUI_String[Lang_x][6],LIGHT);							//显示蓝牙信号
	else	OLED_Put12x12CNstr(98,39,StandbyUI_String[Lang_x][7],LIGHT);											//显示蓝牙信号
}	
...
void Remote_Mode(void)
{
	if(UI_Display_Flag==1)
	{
		...
		OLED_Put12x12CNstr(0,26,StandbyUI_String[Lang_x][2],LIGHT);			OLED_Put12x12CNstr(66,26,StandbyUI_String[Lang_x][3],LIGHT);
		OLED_Put12x12CNstr(0,39,StandbyUI_String[Lang_x][4],LIGHT);			OLED_Put12x12CNstr(66,39,StandbyUI_String[Lang_x][5],LIGHT);
		OLED_Put12x12CNstr(0,52,StandbyUI_String2[1][Lang_x],LIGHT);			//显示提示语
...
void Display_Menux(char Menuxx)
{
	...
	if(Menuxx==2)	OLED_Put5x7Char(0,28,'>',LIGHT);
	else					OLED_Put5x7Char(0,28,' ',LIGHT);
	if(Menuxx==3)	OLED_Put5x7Char(0,41,'>',LIGHT);
	else					OLED_Put5x7Char(0,41,' ',LIGHT);
	if(Menuxx==4)	OLED_Put5x7Char(0,54,'>',LIGHT);
	else					OLED_Put5x7Char(0,54,' ',LIGHT);
	
	if(Menuxx==5)	OLED_Put5x7Char(69,15,'>',LIGHT);
	else					OLED_Put5x7Char(69,15,' ',LIGHT);
	if(Menuxx==6)	OLED_Put5x7Char(69,28,'>',LIGHT);
	else					OLED_Put5x7Char(69,28,' ',LIGHT);
	if(Menuxx==7)	OLED_Put5x7Char(69,41,'>',LIGHT);
	else					OLED_Put5x7Char(69,41,' ',LIGHT);
	if(Menuxx==8)	OLED_Put5x7Char(69,54,'>',LIGHT);
	else					OLED_Put5x7Char(69,54,' ',LIGHT);
}
...
void Settings_Mode(void)
{
	if(UI_Display_Flag==1)	
	{
		OLED_ClearBuffer(0);
		OLED_Put12x12CNstr(24,0,Title_Strings[WorkMode][Lang_x],LIGHT);	
		OLED_Put12x12CNstr(6,13,SettingUI_String[Lang_x][0],LIGHT);		OLED_Put12x12CNstr(75,13,SettingUI_String[Lang_x][1],LIGHT);
		OLED_Put12x12CNstr(6,26,SettingUI_String[Lang_x][2],LIGHT);		OLED_Put12x12CNstr(75,26,SettingUI_String[Lang_x][3],LIGHT);
		OLED_Put12x12CNstr(6,39,SettingUI_String[Lang_x][4],LIGHT);		OLED_Put12x12CNstr(75,39,SettingUI_String[Lang_x][5],LIGHT);
		OLED_Put12x12CNstr(6,52,SettingUI_String[Lang_x][6],LIGHT);		OLED_Put12x12CNstr(75,52,SettingUI_String[Lang_x][7],LIGHT);
...
void PID_Settings(void)
{
	...
	{	
		OLED_ClearBuffer(0);
		OLED_Put12x12CNstr(24,0,Title_Strings[WorkMode][Lang_x],LIGHT);	
		OLED_Put12x12CNstr(0,13,PIDsetUI_String[Lang_x][0],LIGHT);OLED_Put12x12CNstr(24,13,"P：   I：   D：  ",LIGHT);
//		OLED_Put12x12CNstr(0,26,PIDsetUI_String[Lang_x][1],LIGHT);OLED_Put12x12CNstr(24,26,"P：   I：   D：  ",LIGHT);
//		OLED_Put12x12CNstr(0,39,PIDsetUI_String[Lang_x][2],LIGHT);OLED_Put12x12CNstr(24,39,"P：   I：   D：  ",LIGHT);
...
//		OLED_ClearBuffer(0);
//		OLED_Put12x12CNstr(24,0,Title_Strings[WorkMode][Lang_x],LIGHT);	
//		OLED_Put12x12CNstr(3,13,CALI_String[Lang_x][0],LIGHT);
//		OLED_Put12x12CNstr(3,26,CALI_String[Lang_x][1],LIGHT);
//		OLED_Put12x12CNstr(3,39,CALI_String[Lang_x][2],LIGHT);
//		OLED_Put12x12CNstr(3,52,CALI_String[Lang_x][3],LIGHT);
//		LED_state=LED_FASTFLASH;Menux=1;LeftKnob=0;RightKnob=0;	
//		UI_Display_Flag=0;
//	}
//	if(Kaysta==1)	//按键短按
...
//	{
//		for(nx=0;nx<16;nx++)
//			OLED_DrawLine_X(22,106,10+nx,LIGHT);
//		OLED_Put12x12CNstr(22,26,MasBox_String[Lang_x][1],DARK);
//		for(nx=28;nx<44;nx++)
//			OLED_DrawLine_X(22,106,10+nx,LIGHT);
//		
...
//		for(nx=0;nx<16;nx++)
//			OLED_DrawLine_X(22,106,10+nx,LIGHT);
//		OLED_Put12x12CNstr(22,26,"  ",DARK);
//		OLED_PutChar0612(34,26,AdjustAngle_Flag/200+1+48,DARK);
//		OLED_Put12x12CNstr(40,26,CALI_String2[Lang_x],DARK);
//		for(nx=28;nx<44;nx++)
//			OLED_DrawLine_X(22,106,10+nx,LIGHT);
//	}
}
...
void Display_SaveMasBox(void)
{
	unsigned char mx;
	for(mx=0;mx<16;mx++)
		OLED_DrawLine_X(22,106,10+mx,LIGHT);
	OLED_Put12x12CNstr(22,26,MasBox_String[Lang_x][0],DARK);
	for(mx=28;mx<44;mx++)
		OLED_DrawLine_X(22,106,10+mx,LIGHT);
}
...
{
	if(Menuxx==1)	OLED_Put12x12CNstr(6,13,">",LIGHT);
	else 	OLED_Put12x12CNstr(6,13," ",LIGHT);
	if(Menuxx==2)	OLED_Put12x12CNstr(6,26,">",LIGHT);
	else	OLED_Put12x12CNstr(6,26," ",LIGHT);
	if(Menuxx==3)	OLED_Put12x12CNstr(6,39,">",LIGHT);
	else	OLED_Put12x12CNstr(6,39," ",LIGHT);
} 
...
void LanguageSettings_Mode(void)
{
	...
	{	
		OLED_ClearBuffer(0);
		OLED_Put12x12CNstr(24,0,Title_Strings[WorkMode][Lang_x],LIGHT);	
		OLED_Put12x12CNstr(24,13,"简体中文",LIGHT);
		OLED_Put12x12CNstr(24,26,"繁體中文",LIGHT);
		OLED_Put12x12CNstr(24,39,"English",LIGHT);
		LED_state=LED_FASTFLASH;Menux=1;LeftKnob=0;RightKnob=0;	
		UI_Display_Flag=0;
	}
	if(LeftKnob>150)
...
void Display_FactoryBox(void)
{
	...
	for(mx=0;mx<16;mx++)
		OLED_DrawLine_X(22,106,10+mx,LIGHT);
	OLED_Put12x12CNstr(22,26,MasBox_String[Lang_x][2],DARK);
	for(mx=28;mx<44;mx++)
		OLED_DrawLine_X(22,106,10+mx,LIGHT);
}
...
void FactorySettings_Mode(void)
{
	if(UI_Display_Flag==1)	
	{
		OLED_ClearBuffer(0);
		OLED_Put12x12CNstr(24,0,Title_Strings[WorkMode][Lang_x],LIGHT);	
		OLED_Put12x12CNstr(3,26,Factory_String[Lang_x],LIGHT);
		LED_state=LED_FASTFLASH;Menux=1;LeftKnob=0;RightKnob=0;	
		UI_Display_Flag=0;
	}
	if(LeftKnob>150)
	...
	switch(Menux)
	{
		case 1://否
			...
			if(Lang_x<2)
			{	OLED_Put12x12CNstr(30,52,"否",DARK);OLED_Put12x12CNstr(86,52,"是",LIGHT);	}
			else
			{	OLED_Put12x12CNstr(30,52,"NO",DARK);OLED_Put12x12CNstr(83,52,"YES",LIGHT);	}			
			break;
		case 2://是
			if(Kaysta==1)	//按键短按
			{					
			...
			if(Lang_x<2)
			{	OLED_Put12x12CNstr(30,52,"否",LIGHT);OLED_Put12x12CNstr(86,52,"是",DARK);	 }
			else
			{	OLED_Put12x12CNstr(30,52,"NO",LIGHT);OLED_Put12x12CNstr(83,52,"YES",DARK);}				
			break;
		default:
...
void Other_Settings_Mode(void)
{
	if(UI_Display_Flag==1)	
	{
		OLED_ClearBuffer(0);
		OLED_Put12x12CNstr(24,0,Title_Strings[WorkMode][Lang_x],LIGHT);
//		OLED_Put12x12CNstr(0,17,SafeAngleUI_String[Lang_x][2],LIGHT);
//		OLED_Put12x12CNstr(0,32,SafeAngleUI_String[Lang_x][2],LIGHT);
...
void VoltWarn_Mode(void)
{
	...
	for(mx=0;mx<16;mx++)
		OLED_DrawLine_X(22,106,10+mx,LIGHT);
	OLED_Put12x12CNstr(22,26,MasBox_String[Lang_x][3],DARK);
	for(mx=28;mx<44;mx++)
		OLED_DrawLine_X(22,106,10+mx,LIGHT);	
	VoltLowTimes-=2;
	if(VoltLowTimes==0) 
