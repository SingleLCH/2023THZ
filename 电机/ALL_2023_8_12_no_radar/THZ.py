...
insertPicture = "INSERT INTO ad_customer (c_id, c_name, time, c_pic, c_test) VALUES (%s, %s, %s, %s, %s)"
...
def poll(conn, cursor):
    ...
    if w!=1:
        ...
        try:
            ...
            # 选择 z-score 值小于 1 的点
            threshold = 20
            good_points = np.abs(z_scores) < threshold

            # 去除异常点
            points_clean = points[good_points]
            values_clean = values[good_points]
            ...
            ax.imshow(zi, extent=(min(x), max(x), min(y), max(y)), origin='lower', cmap='gray')
            ax.axis('off')
        ...
        for file in files:
            ...
            for png in png_files:
                ...
                with open(path, 'rb') as p:
                    ...
                    # Insert image data into database
                    cursor.execute(insertPicture, (cid, name, filename, img_base64, img_base))
                    print("Success test")
                    conn.commit()
